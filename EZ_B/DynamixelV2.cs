...
namespace EZ_B {
  ...
  public class DynamixelV2 {

    EZB _ezb;

    // Dynamixel 2.0 communication protocol used by Dynamixel XL-320 and Dynamixel PRO only.    
    // uses 2.0 protocol http://support.robotis.com/en/techsupport_eng.htm#product/dynamixel2/communication/xl-320.htm
    // http://support.robotis.com/en/techsupport_eng.htm#product/dynamixel2/communication/xl-320.htm
    // https://github.com/HelloSpoon/Arduino-XL320

    protected internal DynamixelV2(EZB ezb) {
    ...
    public static byte[] ServoSpeed(byte id, int speed) {
      ...
      return CreateDynamixelCommand(id, 3, 34, speed);
    }

    private static UInt16 update_crc(byte[] data_blk_ptr) {
    ...
    private static byte[] CreateDynamixelCommand(byte ID, byte command, byte Address, int value) {
      ...
      txbuffer.Add(command);

      List<byte> pList = new List<byte>();
      pList.AddRange(BitConverter.GetBytes((UInt16)Address));
      pList.AddRange(BitConverter.GetBytes((UInt16)value));

      txbuffer.AddRange(pList.ToArray());

      UInt16 wchecksum = update_crc(txbuffer.ToArray());

      txbuffer.AddRange(BitConverter.GetBytes(wchecksum));
    ...
    public void LED(byte id, bool status) {
      ...
      SendCommandToEZB(CreateDynamixelCommand(id, 3, 25, status ? 1 : 0));
    }
    ...
    public static byte[] GetDisableStatusPacketCmd(byte id) {
      ...
      return CreateDynamixelCommand(id, 0x03, 0x10, 0);
    }
